...
function defaultDetectRemote(path) {
    ...
    return path.some((segment) => {
        ...
        return normalized === 'cleaker' || normalized.includes('cleaker:') || normalized.includes(':');
    });
}
...
export function bindKernel(me, options = {}) {
    ...
    async function open(input) {
        ...
        else if (typeof me.replayMemories === 'function' && hydratedMemories.length > 0) {
    ...
    function getOrCreateRemoteSlot(path) {
        ...
        slot.promise = remotePointer.resolve(resolverConfig.resolveOptions).then((resolved) => {
            ...
            if (resolved.ok) {
                ...
                remoteOverlay.set(key, applied === undefined ? unwrapResolvedValue(resolved.data) : applied);
            }
            return resolved;
        });
