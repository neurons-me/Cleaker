**cleaker**

***

# cleaker

## Interfaces

- [CleakerErrorPayload](interfaces/CleakerErrorPayload.md)
- [CleakerEvents](interfaces/CleakerEvents.md)
- [CleakerNode](interfaces/CleakerNode.md)
- [CleakerOptions](interfaces/CleakerOptions.md)
- [CleakerReadyPayload](interfaces/CleakerReadyPayload.md)
- [CleakerStatus](interfaces/CleakerStatus.md)
- [MeKernel](interfaces/MeKernel.md)
- [NamespaceFailedPayload](interfaces/NamespaceFailedPayload.md)
- [NamespaceFallbackPayload](interfaces/NamespaceFallbackPayload.md)
- [OpenNodeInput](interfaces/OpenNodeInput.md)
- [OpenNodeResult](interfaces/OpenNodeResult.md)
- [ParsedNamespaceExpression](interfaces/ParsedNamespaceExpression.md)
- [ParsedTarget](interfaces/ParsedTarget.md)
- [ParseTargetOptions](interfaces/ParseTargetOptions.md)
- [RegisterSurfaceInput](interfaces/RegisterSurfaceInput.md)
- [RemotePointerDefinition](interfaces/RemotePointerDefinition.md)
- [ResolvePointerOptions](interfaces/ResolvePointerOptions.md)
- [ResolvePointerResult](interfaces/ResolvePointerResult.md)
- [ResolveSurfaceInput](interfaces/ResolveSurfaceInput.md)
- [SurfaceEndpoint](interfaces/SurfaceEndpoint.md)
- [TopologyResolver](interfaces/TopologyResolver.md)

## Type Aliases

- [SurfaceTransport](type-aliases/SurfaceTransport.md)

## Functions

- [composeNamespace](functions/composeNamespace.md)
- [default](functions/default.md)
- [parseNamespaceExpression](functions/parseNamespaceExpression.md)
- [parseTarget](functions/parseTarget.md)
- [stringifyNamespaceExpression](functions/stringifyNamespaceExpression.md)
