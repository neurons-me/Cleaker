import { createNamespaceRegion, namespaceRegionToString, parseNamespaceRegion, } from '../algebra';
import { parseTarget } from './parseTarget';
const DEFAULT_LOCAL_PREFIXES = ['docs', 'assets'];
export function parseBrowserNamespace(input, options = {}) {
    const raw = String(input ?? '').trim();
    if (!raw)
        throw new Error('Browser namespace cannot be empty');
...
export function browserNamespaceToString(parsed) {
    if (parsed.kind === 'nrp') {
...
//# sourceMappingURL=parseBrowserNamespace.js.map